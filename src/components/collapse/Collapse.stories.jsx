...
const collapse = props => (
    <Collapse {...props}>
        ...
        <Collapse.Item title="Consistência" name="consistencia">
            <div>A transação cria um novo estado válido dos dados ou em caso de falha retorna todos
                os
                dados ao seu estado antes que a transação foi iniciada.
            </div>
        </Collapse.Item>
        <Collapse.Item title="Isolamento" name="isolamento">
            <div>Uma transação em andamento mas ainda não validada deve permanecer isolada de
                qualquer
        ...
        <Collapse.Item title="Durabilidade" name="durabilidade">
            <div>Dados validados são registados pelo sistema de tal forma que mesmo no caso de uma
                falha
...
storiesOf('Outros|Collapse', module)
    .add('default', () => {
        return collapse({ activeName: 'consistencia' });
    })

    .add('accordion', () => {
        return collapse({ activeName: 'atomicidade', accordion: true });
    })
