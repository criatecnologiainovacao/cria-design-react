...
export default class Pager extends Component {
    ...
    constructor(props: Object, context: Object) {
        ...
        this.state = {
            ...
            quickprevIconClass: 'cd-icon-more',
            quicknextIconClass: 'cd-icon-more',
            showPrevMore: false,
            showNextMore: false
        };
    }
    ...
    render(): React.DOM {
        ...
        return (
          <ul onClick={this.onPagerClick.bind(this)} className="cd-pager">
            {pageCount > 0 && (
              <li
            ...
            {this.state.showPrevMore && (
              <li
                className={this.classNames(
                        'cd-icon more btn-quickprev',
                        quickprevIconClass
                    )}
                onMouseEnter={() => {
                        this.setState({ quickprevIconClass: 'cd-icon-d-arrow-left' });
                    }}
                onMouseLeave={() => {
                        this.setState({ quickprevIconClass: 'cd-icon-more' });
                    }}
                />
)}
            ...
            {this.state.showNextMore && (
              <li
                className={this.classNames(
                        'cd-icon more btn-quicknext',
                        quicknextIconClass
                    )}
                onMouseEnter={() => {
                        this.setState({ quicknextIconClass: 'cd-icon-d-arrow-right' });
                    }}
                onMouseLeave={() => {
                        this.setState({ quicknextIconClass: 'cd-icon-more' });
                    }}
                />
)}
